...
const SharedGame = () => {
    ...
    useEffect(() => {
        const fetchJeopardyData = async () => {
            try {
                ...
                const { data, title, high_score } = await response.json();
                console.log(data)
    ...
    return <PlayScreen categories={jeopardyData} handleReset={handleNewGame} title={gameTitle} link={link} highScore={highScore} isPublic={false}/>;
}
